package com.mycustomappapply.wotttoo.ui.main
...
import com.mycustomappapply.wotttoo.utils.isRootScreen
import com.mycustomappapply.wotttoo.utils.shouldShowBottomNav
...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun setupDrawerUi(): Unit = with(binding) {
        val currentUser: UserAuth = authViewModel.getUser()
        if (authViewModel.isAuthenticated) {
            val headerView: View = drawerNavigationView.getHeaderView(0)
            val headerBinding: DrawerHeaderLayoutBinding = DrawerHeaderLayoutBinding.bind(headerView)
            ...
            val drawerMenu: Menu = drawerNavigationView.menu
            drawerMenu.findItem(R.id.drawer_login).isVisible = false
        } else {
            val bottomNavMenu: Menu = drawerNavigationView.menu
    ...
    private fun shareApp() {
        try {
            ...
            shareIntent.putExtra(Intent.EXTRA_SUBJECT, "wotttoo")
            var shareMessage =
    ...
    private fun setupNavigation(): Unit = with(binding) {
        ...
        bottomNavView.setOnItemReselectedListener {
            val screen: Screen? = when (it.itemId) {
